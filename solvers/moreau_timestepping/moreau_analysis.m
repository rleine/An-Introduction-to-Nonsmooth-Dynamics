function [t,q,u,PN,PT] = moreau_analysis(sys,t0,te,q0,u0,N,tol,maxiter)
...
%         q          position
%         u          velocity
%         PN         contact effort normal direction
%         PT         contact effort tangential direction
%
% Remco Leine, October 2024
...
for i = 1:(N-1)
    ...
    if ~isempty(IC)
        ...
        converged = 0; 
        k = 0;
        PN_IC = PN(IC,i); % warm-start with last time-step
        PT_IC = PT(IC,i);
        while (~converged && k < maxiter) % fixed point iteration
            uE = uB  + MM\(hM*dt + WN(:,IC)*PN_IC + WT(:,IC)*PT_IC);
            ...
            PN_new = -prox_CN(-PN_IC + sys.rN*xiN(IC));
            PT_new = -prox_CT(-PT_IC + sys.rT*xiT(IC),mu(IC).*PN_new);
            error = max(abs(PN_IC-PN_new)) + max(abs(PT_IC-PT_new)); 
            PN_IC = PN_new;
            PT_IC = PT_new;
            converged = (error < tol);
            k = k + 1;
